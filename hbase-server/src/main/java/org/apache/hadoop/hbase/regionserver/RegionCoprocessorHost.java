...
public class RegionCoprocessorHost
  ...
  public InternalScanner preFlushScannerOpen(final Store store,
      final List<KeyValueScanner> memstoreScanners, final long readPoint) throws IOException {
    return execOperationWithResult(null,
        coprocessors.isEmpty() ? null : new RegionOperationWithResult<InternalScanner>() {
      @Override
      public void call(RegionObserver oserver, ObserverContext<RegionCoprocessorEnvironment> ctx)
          throws IOException {
        setResult(oserver.preFlushScannerOpen(ctx, store, memstoreScanners, getResult(), readPoint));
      }
    });
  }
