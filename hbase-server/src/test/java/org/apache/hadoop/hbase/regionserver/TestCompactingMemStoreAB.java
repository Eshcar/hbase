...
public class TestCompactingMemStoreAB extends TestCompactingMemStore {

  private static final Log LOG = LogFactory.getLog(TestCompactingMemStoreAB.class);
  ...
  @Override public void setUp() throws Exception {
    ...
    this.memstore =
        new CompactingMemStore(HBaseConfiguration.create(), CellComparator.COMPARATOR, store,
            regionServicesForStores, CompactingMemStore.Type.COMPACT_TO_ARRAY_MAP);
  ...
  public void testGetNextRow() throws Exception {
    ...
    assertTrue(KeyValue.COMPARATOR
        .compareRows(closestToEmpty, new KeyValue(Bytes.toBytes(0), System.currentTimeMillis())) == 0);
    for (int i = 0; i < ROW_COUNT; i++) {
      ...
      if (i + 1 == ROW_COUNT) {
        ...
        assertTrue(KeyValue.COMPARATOR
            .compareRows(nr, new KeyValue(Bytes.toBytes(i + 1), System.currentTimeMillis())) == 0);
      }
    }
    ...
    for (int startRowId = 0; startRowId < ROW_COUNT; startRowId++) {
      ScanInfo scanInfo =
          new ScanInfo(conf, FAMILY, 0, 1, Integer.MAX_VALUE, KeepDeletedCells.FALSE, 0, this.memstore.getComparator());
      ScanType scanType = ScanType.USER_SCAN;
      InternalScanner scanner =
          new StoreScanner(new Scan(Bytes.toBytes(startRowId)), scanInfo, scanType, null, memstore.getScanners(0));
  ...
  public void testUpsertMSLAB() throws Exception {
    ...
    System.out.println(
        "Memory used: " + (usageAfter - usageBefore) + " (heapsize: " + memstore.heapSize() +
            " size: " + size + ")");
  }
  ...
  public void testCompaction1Bucket() throws IOException {
    ...
    assertEquals(0, memstore.getSnapshot().getCellsCount());
    assertEquals(344, regionServicesForStores.getGlobalMemstoreTotalSize());
...
//            + ". This is the memstore flushable size - " + size + "\n",false);
    ...
    assertEquals(480, regionServicesForStores.getGlobalMemstoreTotalSize());
  ...
  public void testCompaction3Buckets() throws IOException {
    ...
    assertEquals(1400, regionServicesForStores.getGlobalMemstoreTotalSize());
